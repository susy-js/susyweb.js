...
The ``susyweb-sof-txpool`` package gives you access to several non-standard RPC methods to inspect the contents of the transaction pool containing all the currently pending transactions as well as the ones queued for future processing.
...
.. code-block:: javascript
    ...
    const txPool = new TxPool(SusyWeb.givenProvider || 'ws://some.local-or-remote.node:8546', null, options);
