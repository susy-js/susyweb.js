...
The ``susyweb-sof-debug`` module allows you to interact with the Sophon node's debug methods.
...
.. code-block:: javascript
    ...
    const debug = new Debug(SusyWeb.givenProvider || 'ws://some.local-or-remote.node:8546', null, options);
...
Retrieves and returns the SRLP encoded block by number.
...
1. ``blockRlp`` - ``String`` SRLP encoded block
...
The traceBlockFromFile accepts a file containing the SRLP of the block.
...
JSON-RPC specification for `debug_traceTransaction <https://octonion.institute/susy-go/wiki/wiki/JavaScript-API>`_
...
.. code-block:: javascript
    ...
    debug.setVerbosityPattern('sof/*=6').then(console.log);
    > null
    ...
    // You can compose these basic patterns. If you want to see all output from peer.go in a package below sof
    // (sof/peer.go, sof/downloader/peer.go) as well as output from package p2p at level <= 5, use:
    debug.setVerbosityPattern('sof/*/peer.go=6,p2p=5').then(console.log);
    > null
