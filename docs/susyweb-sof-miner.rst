...
The ``susyweb-sof-miner`` package allows you to remote control the node's mining operation and set various mining specific settings.
...
.. code-block:: javascript
    ...
    const miner = new Miner(SusyWeb.givenProvider || 'ws://some.local-or-remote.node:8546', null, options);
...
.. code-block:: javascript
    ...
    miner.setSophyBase(address, [, callback])

Sets sophybase, where mining reward will go.
The RPC method used is ``miner_setSophybase``.
...
.. code-block:: javascript
    ...
    miner.setSophyBase("0x3d80b31a78c30fc628f20b2c89d7ddbf6e53cedc").then(console.log);
    > true
