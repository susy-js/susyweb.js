...
The ``susyweb.sof.subscribe`` function lets you subscribe to specific events in the blockchain.
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe(type [, options] [, callback]);
...
.. code-block:: javascript
    ...
    const subscription = susyweb.sof.subscribe('logs', {
        address: '0x123456..',
        topics: ['0x12345...']
...
.. code-block:: javascript
    ...
    susyweb.sof.clearSubscriptions()

Resets subscriptions.

.. note:: This will not reset subscriptions from other packages like ``susyweb-shh``.
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe('logs', {} ,function(){ ... });
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe('pendingTransactions' [, callback]);
...
``EventEmitter``: An :ref:`subscription instance <sof-subscription-return>` as an event emitter with the following events:
...
.. code-block:: javascript
    ...
    const subscription = susyweb.sof.subscribe('pendingTransactions', function(error, result){
        if (!error)
            console.log(result);
    })
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe('newBlockHeaders' [, callback]);
...
.. code-block:: javascript
    ...
    const subscription = susyweb.sof.subscribe('newBlockHeaders', function(error, result){
        if (!error) {
            console.log(result);
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe('syncing' [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`susyweb.sof.isSyncing return values <sof-issyncing-return>`.
...
.. code-block:: javascript
    ...
    const subscription = susyweb.sof.subscribe('syncing', function(error, sync){
        if (!error)
            console.log(sync);
    })
...
.. code-block:: javascript
    ...
    susyweb.sof.subscribe('logs', options [, callback]);
...
For the structure of a returned event ``Object`` see :ref:`susyweb.sof.getPastEvents return values <sof-getpastlogs-return>`.
...
1. ``Object|Null`` - First parameter is an error object if the subscription failed.
2. ``Object`` - The log object like in :ref:`susyweb.sof.getPastEvents return values <sof-getpastlogs-return>`.
