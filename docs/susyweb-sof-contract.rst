...
The ``susyweb.sof.Contract`` object makes it easy to interact with smart contracts on the Sophon blockchain.
When you create a new contract object you give it the json interface of the respective smart contract
and susyweb will auto convert all calls into low level ABI calls over RPC for you.
...
.. code-block:: javascript
    ...
    new susyweb.sof.Contract(jsonInterface, address, options)
...
.. code-block:: javascript
    ...
    const myContract = new susyweb.sof.Contract([...], '0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe', {
...
All transactions generated by susyweb.js from this contract will contain this address as the "to".
...
The :ref:`json interface <glossary-json-interface>` object derived from the `ABI <https://octonion.institute/susy-go/wiki/wiki/Sophon-Contract-ABI>`_ of this contract.
...
.. code-block:: javascript
    ...
    const contract1 = new sof.Contract(abi, address, {gasPrice: '12345678', defaultAccount: fromAddress});
...
- ``Array`` - arguments: The arguments passed to the method before. They can be changed.
- ``Function`` - :ref:`call <contract-call>`: Will call the "constant" method and execute its smart contract method in the SVM without sending a transaction (Can't alter the smart contract state).
...
Will call a "constant" method and execute its smart contract method in the SVM without sending any transaction. Note calling can not alter the smart contract state.
...
.. code-block:: javascript
    ...
    // susyweb.js
    const MyContract = new susyweb.sof.Contract(abi, address);
...
Will call estimate the gas a method execution will take when executed in the SVM without.
