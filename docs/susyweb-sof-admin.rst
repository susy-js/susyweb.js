...
The ``susyweb-sof-admin`` package allows you to interact with the Sophon node's admin management.
...
.. code-block:: javascript
    ...
    const admin = new Admin(SusyWeb.givenProvider || 'ws://some.local-or-remote.node:8546', null, options);
...
Add an admin peer on the node that SusyWeb is connected to with its provider.
...
``Promise<object>`` - The node information array.
    ...
    - ``listener`` - ``number``: TCP listening port for SRLPx
    ...
    - ``network`` - ``number``: currently used Sophon networks ids.
...
.. code-block:: javascript
    ...
    > {
        ...
        ip: "::",
        listenAddr: "[::]:30303",
        name: "Graviton/v1.5.0-unstable/linux/go1.6",
        ports: {
            discovery: 30303,
            listener: 30303
        },
...
.. code-block:: javascript
    ...
    > [{
            ...
            name: "Graviton/v1.5.0-unstable/linux/go1.5.1",
            network: {
            ...
            name: "Graviton/v1.3.5-506c9277/linux/go1.4.2",
            network: {
...
Sets the Solidity compiler path to be used by the node when invoking the sof_compileSolidity RPC method
...
4. ``apis`` - ``string`` -  (optional) API modules to offer over this interface (defaults to "sof,net,susyweb").
