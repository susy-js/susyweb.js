...
jest.mock('../../../src/providers/SusyWebSophonProvider');
...
describe('ProviderResolverTest', () => {
    ...
    it('calls resolve with the SophonProvider', () => {
        const sophonProviderMock = {};
        sophonProviderMock.isSIP1193 = true;

        providersModuleFactoryMock.createSusyWebSophonProvider.mockReturnValueOnce(sophonProviderMock);

        expect(providerResolver.resolve(sophonProviderMock)).toEqual(sophonProviderMock);

        expect(providersModuleFactoryMock.createSusyWebSophonProvider).toHaveBeenCalledWith(sophonProviderMock);
    });
