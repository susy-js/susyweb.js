...
import * as SofLibAccount from 'sof-lib/lib/account'; // TODO: Remove this dependency
import uuid from 'uuid';
import Hash from 'sof-lib/lib/hash';
import randomBytes from 'randombytes';
import {pbkdf2Sync} from 'pbkdf2';
import {createCipheriv, createDecipheriv} from 'browserify-cipher';
import {isHexStrict, hexToBytes, randomHex, keccak256} from 'susyweb-utils'; // TODO: Use the VO's of a susyweb-types module.
...
export default class Account {
    ...
    /**
     * TODO: Add deprecation message, remove accounts dependency and extend the signTransaction method in the sof module.
    ...
    sign(data) {
        ...
        const preamble = `\u0019Sophon Signed Message:\n${data.length}`;
        const preambleBuffer = Buffer.from(preamble);
        const sofMessage = Buffer.concat([preambleBuffer, messageBuffer]);
        const hash = Hash.keccak256s(sofMessage);
        const signature = SofLibAccount.sign(hash, this.privateKey);
        const vrs = SofLibAccount.decodeSignature(signature);
    ...
    static from(entropy, accounts = {}) {
        return new Account(SofLibAccount.create(entropy || randomHex(32)), accounts);
    }
    ...
    static fromPrivateKey(privateKey, accounts = {}) {
        return new Account(SofLibAccount.fromPrivate(privateKey), accounts);
    }
