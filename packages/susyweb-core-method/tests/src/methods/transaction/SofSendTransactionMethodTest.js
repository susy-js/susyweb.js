...
/**
 * SofSendTransactionMethod test
 */
describe('SofSendTransactionMethodTest', () => {
    let method,
        providerMock,
        moduleInstanceMock,
    ...
    beforeEach(() => {
        ...
        new AbstractSusyWebModule(providerMock, {}, {}, {});
        moduleInstanceMock = AbstractSusyWebModule.mock.instances[0];
        ...
        method = new SofSendTransactionMethod(
            Utils,
            formatters,
            moduleInstanceMock,
            transactionObserverMock,
    ...
    it('calls the static property Type and it returns the expect value', () => {
        expect(SofSendTransactionMethod.Type).toEqual('sof-send-transaction-method');
    });
    ...
    it('calls execute and the gasPrice will be defined with "sof_gasPrice" and returns with a resolved promise', (done) => {
    ...
    it('calls execute and the gasPrice will be defined with "sof_gasPrice" and returns with a reject promise', async () => {
        providerMock.send = jest.fn(() => {
        ...
        expect(providerMock.send).toHaveBeenNthCalledWith(1, 'sof_gasPrice', []);
    });
    ...
    it('calls execute and signs on the node', (done) => {
        ...
        method.callback = (error, hash) => {
            ...
            expect(providerMock.send).toHaveBeenCalledWith('sof_sendTransaction', parameters);
