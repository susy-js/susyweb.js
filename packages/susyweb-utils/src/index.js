 ...
 * @author Fabian Vogelsteller <fabian@sophon.org>
 * @date 2017
 */
...
import * as sofjsUnit from 'sofjs-unit';
...
export const fromWei = (number, unit) => {
    ...
    return utils.isBN(number) ? sofjsUnit.fromWei(number, unit) : sofjsUnit.fromWei(number, unit).toString(10);
};
...
export const toWei = (number, unit) => {
    ...
    return utils.isBN(number) ? sofjsUnit.toWei(number, unit) : sofjsUnit.toWei(number, unit).toString(10);
};
...
export const toChecksumAddress = (address, chainId = null) => {
    ...
    if (!/^(0x)?[0-9a-f]{40}$/i.test(address))
        throw new Error(`Given address "${address}" is not a valid Sophon address.`);
